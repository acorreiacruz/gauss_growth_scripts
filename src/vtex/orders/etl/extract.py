...
class Extract(DateTimeConfig):
    ...
    def __get_orders_ids(self, orders: list) -> list:
        return [order["orderId"] for order in orders]

    def __get_detailed_orders(self, order_ids: list) -> list:
        orders = []
        for id in order_ids:
    ...
    def run(self) -> List[dict]:
        ...
        yesterday_vtex_orders = self.__get_orders()
        if not yesterday_vtex_orders:
            return []
        print(f"Número de pedidos {len(yesterday_vtex_orders)}")
        vtex_orders_ids = self.__get_orders_ids(orders=yesterday_vtex_orders)
        print("Obtendo os pedidos de forma detalhada ...")
        orders = self.__get_detailed_orders(order_ids=vtex_orders_ids)
        return orders
